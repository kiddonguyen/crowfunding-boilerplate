 ...
 * @param {boolean} param0.open - Indicates whether the eye is open or closed.
 * @param {function} param0.onClick - Function to be called when the icon is clicked.
 * @returns {JSX.Element} - Returns the Eye icon as an SVG.
 */
const IconEyeToggle     = ({ open = false, onClick = () => {} }) => {
  if (!open) {
    return (
      // SVG code for the closed eye
      <svg
        width={24}
        height={24}
        viewBox="0 0 24 24"
